...
class SubscriptionPublicViewSet(UserOrManagedMixin, GenericViewSet):
    ...
    def request_subscribe(self, request):
        ...
        text = render_to_string("newsletter/email/sub_request.txt", ctx)
        html = render_to_string("newsletter/email/sub_request.html", ctx)
    ...
    def request_manage_link(self, request):
        ...
        manage_claim_url = f"{settings.SITE_URL}/news/manage/claim/{token}/"

        ctx = {"manage_url": manage_claim_url}
        subject = "Manage your subscriptions"
        text = render_to_string("newsletter/email/manage_link.txt", ctx)
        html = render_to_string("newsletter/email/manage_link.html", ctx)
    ...
    def list_current(self, request):
        ...
        for nl in Newsletter.objects.public():
...
class AdminSubscriberViewSet(
    ...
    - POST   /api/v2/news/admin/subscribers/{id}/resubscribe/
    - DELETE /api/v2/news/admin/subscribers/{id}/erase/
